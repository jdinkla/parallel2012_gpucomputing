 ...
 * 
 * Bemerkung: 3D-Code mit Streaming und zwei Threads.
 */
#include <omp.h>
// CUDA
...
void smoothing3d_stream4(const CExtent& extentHost, 
	...
	it2.next(); // Iterator 2 is one steap ahead (even instead of odd)
	...
	#pragma omp parallel sections
	{
		...
		#pragma omp section
		{
			while (it.next()) {
				...
				cout << "------------------------------------------------------------" << endl;
				cout << "T1: Iteration: " << it.getCurrentStart() << " - " << it.getCurrentEnd() << " (" << it.getCurrentSize() << ")" << endl;
				cout << "T1: Padded:    " << it.getCurrentStartPadded() << " - " << it.getCurrentEndPadded() << " (" << it.getCurrentSizePadded() << ")"  << endl;
				cout << "T1: Offset:    " << offset1 << endl;
				...
				smooth3d_swap(config1, *input1.device, *output1.device, windowSize, offset1, realExtent);
		...
		#pragma omp section
		{
			while (it2.next()) {
				...
				cout << "------------------------------------------------------------" << endl;
				cout << "T2: Iteration: " << it2.getCurrentStart() << " - " << it2.getCurrentEnd() << " (" << it2.getCurrentSize() << ")" << endl;
				cout << "T2: Padded:    " << it2.getCurrentStartPadded() << " - " << it2.getCurrentEndPadded() << " (" << it2.getCurrentSizePadded() << ")"  << endl;
				cout << "T2: Offset:    " << offset2 << endl;
				...
				smooth3d_swap(config2, *input2.device, *output2.device, windowSize, offset2, realExtent);

				// Kopiere das Ergebnis zurück zum Host (in den Pinned Buffer)
				output2.updateHost(true);

				// Kopiere vom Pinned Buffer in den großen Buffer
				h_copy_output2.updateHost(it2.getCurrentStart(), it2.getCurrentSize());
