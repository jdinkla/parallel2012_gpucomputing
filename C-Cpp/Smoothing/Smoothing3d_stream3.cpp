 ...
 * 
 * Bemerkung: 3D-Code mit Streaming und Klasse CStream, ansonsten wie Version stream2.
 */
// CUDA
...
void smoothing3d_stream3(const CExtent& extentHost, const CExtent& extentDevice, const CExtent& extentPartition, const int windowSize) {
	...
	CDeviceBuffer<uchar4> d_input1(extentDevice, &stream1.stream);				// Mit Padding
	...
	CDeviceBuffer<uchar4> d_input2(extentDevice, &stream2.stream);				// Mit Padding
	...
	CDeviceBuffer<uchar4> d_output1(extentPartition, &stream1.stream);			// Ohne Padding
	...
	CDeviceBuffer<uchar4> d_output2(extentPartition, &stream2.stream);			// Ohne Padding
	...
	CExecConfig config1(extentPartition, stream1.stream);
	CExecConfig config2(extentPartition, stream2.stream);
